...
# 异常处理窗口期检查
    ...
    g.etf_pool_3 = [
        ...
        # 未来方向 热门ETF  
        "515880.XSHG",   # 通信ETF国泰 
        "588200.XSHG",   # 科创芯片ETF （中韩溢价高）
        "515030.XSHG",   # 新能源车ETF华夏
        "159530.XSHE",   # 机器人ETF易方达  机器人
        "159255.XSHE",   # 通用航空ETF易方  无人机低空经济
        "159206.XSHE",   # 卫星ETF永赢      商业航天
        "159316.XSHE",   # 创新药ETF易方达  创新药（生物突破）
...
def calculate_momentum_metrics_172(context, etf):
    """计算动量指标 - 双重动量（长期动量 × 短期动量）"""
    try:
        lookback_long = g.lookback_days          # 长期窗口（默认25天）
        lookback_short = g.short_lookback_days   # 短期窗口（默认10天）
        
        # 获取足够的历史数据
        max_lookback = max(lookback_long, lookback_short) + 20
        prices = attribute_history(etf, max_lookback, '1d', ['close', 'high'])
        ...
        if g.enable_volume_check:
            ...
            if vol_ratio is not None:
                if len(price_series) >= lookback_long + 1:
                    total_return = price_series[-1] / price_series[-(lookback_long + 1)] - 1
                    annualized = (1 + total_return) ** (250 / lookback_long) - 1
        ...
        # ========== 核心：双重动量 ==========
        # 1. 长期动量（过去 lookback_long 个交易日总收益率）
        if len(price_series) >= lookback_long + 1:
            long_return = price_series[-1] / price_series[-(lookback_long + 1)] - 1
        else:
            return None

        # 2. 短期动量（过去 lookback_short 个交易日总收益率）
        if len(price_series) >= lookback_short + 1:
            short_return = price_series[-1] / price_series[-(lookback_short + 1)] - 1
        else:
            short_return = long_return   # 数据不足时的fallback

        # 绝对动量过滤：只考虑长期动量 > 0 的标的
        if long_return <= 0:
            return None

        # 双重动量得分 = 长期动量 × (1 + 短期动量)
        # 正短期动量会放大得分，负短期动量会降低得分
        score = long_return * (1 + short_return)

        # 得分上下限过滤
        if score <= g.min_score_threshold or score >= g.max_score_threshold:
            return None

        # 计算年化收益率用于诊断展示
        ann_ret_long = (1 + long_return) ** (250 / lookback_long) - 1
        short_ann = (1 + short_return) ** (250 / lookback_short) - 1 if lookback_short > 0 else 0

        return {
            'etf': etf,
            'etf_name': etf,
            'annualized_returns': ann_ret_long,   # 长期年化收益
            'r_squared': 1.0,                     # 兼容字段
            'score': score,                       # 双重动量得分
