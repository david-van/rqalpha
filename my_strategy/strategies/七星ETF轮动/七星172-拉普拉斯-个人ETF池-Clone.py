# 克隆自聚宽文章：https://www.joinquant.com/post/72503
# 标题：ETF轮动23年至今3648%，年化204%，回撤19%
# 作者：dongliu
...
def calculate_momentum_metrics(context, etf):
    ...
    try:
        ...
        if g.enable_premium_filter:
            ...
            if premium is not None:
                if premium > g.premium_threshold:
                    log.info(
                        f"🚫 {etf} {name} 溢价率{premium * 100:.2f}% > {g.premium_threshold * 100:.0f}%，从排名中排除")
                    return None
            else:
                # 无法获取溢价率，跳过该过滤条件（不过滤）
        ...
        if g.use_short_momentum_filter and short_annualized < g.short_momentum_threshold:
            log.debug(
                f"{etf} {name} 短期动量{short_annualized * 100:.1f}% < 阈值{g.short_momentum_threshold * 100:.1f}%，过滤")
            return None
