...
def load_meta(sweep_dir: Path) -> dict | None:
    """读取扫描目录下的 meta.json"""
...
def _get_tag_value(meta: dict, tag: str, dim_index: int = 0):
    """从 meta.json 的 tag_values 中获取指定维度的参数值"""
    tv = meta.get("tag_values", {})
    vals = tv.get(tag)
    if vals is None:
        return None
...
# ============================================================
def build_sensitivity_figure(results: dict, meta: dict) -> go.Figure | None:
    """一维参数敏感性图，从 meta.json 获取 X 轴值"""
    dims = meta["dimensions"]
    param_label = f'{dims[0]["display"]} ({dims[0]["name"]})'

    pairs = []
    for tag, data in results.items():
        val = _get_tag_value(meta, tag, 0)
        if val is not None:
            pairs.append((val, tag, data))
    ...
    for idx, (name, vals, fmt) in enumerate(metrics):
        ...
        fig.add_trace(
            go.Scatter(
                x=x_vals, y=vals, mode="lines+markers+text",
    ...
    fig.add_trace(
        go.Scatter(
            x=x_vals, y=trade_counts, mode="lines+markers+text",
...
def build_grid_heatmap_figure(results: dict, meta: dict) -> go.Figure | None:
    """2D 参数网格热力图，从 meta.json 获取参数值"""
    dims = meta["dimensions"]
    if len(dims) < 2:
        return None

    dim0_name = dims[0]["display"]
    dim1_name = dims[1]["display"]

    dim0_vals = sorted(set(_get_tag_value(meta, tag, 0) for tag in results if _get_tag_value(meta, tag, 0) is not None))
    dim1_vals = sorted(set(_get_tag_value(meta, tag, 1) for tag in results if _get_tag_value(meta, tag, 1) is not None))

    if len(dim0_vals) < 2 or len(dim1_vals) < 2:
        return None

    def build_matrix(metric_key):
        mat = np.full((len(dim0_vals), len(dim1_vals)), np.nan)
        for tag, data in results.items():
            v0 = _get_tag_value(meta, tag, 0)
            v1 = _get_tag_value(meta, tag, 1)
            if v0 is None or v1 is None:
                continue
            i = dim0_vals.index(v0)
            j = dim1_vals.index(v1)
    ...
    fig.add_trace(
        go.Heatmap(
            z=mat_sharpe, x=[_fmt_dim1(t) for t in dim1_vals],
            y=[str(d) for d in dim0_vals],
            text=text_sharpe, texttemplate="%{text}", textfont=dict(size=10),
            colorscale="YlGn", showscale=True, colorbar=dict(title="夏普"),
            hovertemplate=f"{dim0_name}=%{{y}}<br>{dim1_name}=%{{x}}<br>夏普=%{{z:.3f}}<extra></extra>",
        ),
        row=1, col=1,
    )
    ...
    fig.add_trace(
        go.Heatmap(
            z=mat_ret, x=[_fmt_dim1(t) for t in dim1_vals],
            y=[str(d) for d in dim0_vals],
            ...
            hovertemplate=f"{dim0_name}=%{{y}}<br>{dim1_name}=%{{x}}<br>年化收益=%{{z:.2%}}<extra></extra>",
        ),
        row=1, col=2,
    )
    ...
    fig.add_trace(
        go.Heatmap(
            z=mat_dd, x=[_fmt_dim1(t) for t in dim1_vals],
            y=[str(d) for d in dim0_vals],
            ...
            hovertemplate=f"{dim0_name}=%{{y}}<br>{dim1_name}=%{{x}}<br>最大回撤=%{{z:.2%}}<extra></extra>",
        ),
        row=1, col=3,
    )
    ...
    fig.update_xaxes(title_text=dim1_name, row=1, col=1)
    fig.update_xaxes(title_text=dim1_name, row=1, col=2)
    fig.update_xaxes(title_text=dim1_name, row=1, col=3)
...
# HTML 仪表盘组装
# ============================================================
def build_dashboard_html(results: dict, output_path: Path, meta: dict = None) -> None:
...
def main():
        ...
        if BASE_DIR.exists():
            for d in sorted(BASE_DIR.iterdir()):
                if d.is_dir() and (d / "meta.json").exists():
                    meta = load_meta(d)
                    dims = ", ".join(dim["display"] for dim in meta["dimensions"])
                    sweeps.append(f"  {d.name}  ({dims})")
        if sweeps:
            print("可用的参数扫描:")
            print("\n".join(sweeps))
    ...
    if not sweep_dir.exists():
        print(f"扫描目录不存在: {sweep_dir}")
        return

    meta = load_meta(sweep_dir)
    dims = ", ".join(dim["display"] for dim in meta["dimensions"]) if meta else "无元信息"
    ...
    print(f"扫描: {sweep_name} ({dims})")
    print(f"实验数: {len(results)}")

    output_path = sweep_dir / "analysis_dashboard.html"
    build_dashboard_html(results, output_path, meta=meta)
    print(f"\n完成! 用浏览器打开: {output_path}")
